...
#             for j in range(3):
#                 if i == j: continue
#                 value = df[spaces[space][i]].iloc[c] - df[spaces[space][j]].iloc[c]
#                 value = max(0, value)
#                 sums += value
#     spaces_value[space] = sums

num_channels = 1
list_channels = []
# dict_channels = defaultdict(float)
dict_channels = defaultdict(lambda: np.inf)
for space in spaces:
    for color, c in colors.items():
        ...
        for indices in combi:
            if len(indices) == 1:
                ...
                dict_channels[spaces[space][indices[0]]] = min(dict_channels[spaces[space][indices[0]]], single_value)
            elif len(indices) == 2:
